...
% Additional packages not provided by tmlr.sty
...
\begin{figure}[t]
  ...
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{1em}
    ...
    Input $(B,12,101)$ $\to$ Conv blocks (32/64/128/256 filters) $\to$
    AdaptiveAvgPool $\to$ FC(256$\to$128)+LN+ReLU $\to$
    12 per-molecule heads $\to$ $\hat{y}_1,\ldots,\hat{y}_{12}$
  \vspace{1em}}}
...
\begin{itemize}
  ...
  \item \textbf{Batch size:} 32. Device: Apple MPS (M5 Pro) for local runs;
        CUDA V100 for HPC.
\end{itemize}
...
We conduct three experiments, each targeting a distinct reviewer concern:
...
available at \url{https://github.com/bshind87/INARA-ExoplanetAtmosphericRetrieval}. The INARA~ATMOS
